...
.controller('OrmCtrl', function($scope, globals) {
    $scope.calcOrm = function() {
        ...
        $info.orm = $info.orm_1Reps = Math.round(orm) + globals.lbs;
        // Percentages & Reps
        $info.orm_95 = $info.orm_2Reps = Math.round(orm * 0.95) + globals.lbs;
        $info.orm_90 = $info.orm_3Reps = Math.round(orm * 0.90) + globals.lbs;
        $info.orm_85 = Math.round(orm * 0.85) + globals.lbs;
        $info.orm_80 = $info.orm_7Reps = Math.round(orm * 0.80) + globals.lbs;
        $info.orm_75 = $info.orm_10Reps = Math.round(orm * 0.75) + globals.lbs;
        $info.orm_70 = $info.orm_12Reps = $info.orm_7Reps = Math.round(orm * 0.70) + globals.lbs;
